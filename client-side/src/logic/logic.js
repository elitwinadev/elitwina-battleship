...
const aroundSinkUpdate = (board, x, y) => {
  ...
  if (
    ...
    //!= undefined
    if (newBoard[x][y].value !== MISS && newBoard[x][y].value !== SHIP_PART) {
      //we want to keep miss visual, also not to change a sunken ship to a aroung sink bc of how loop works
...
const aroundShipUpdate = (board, x, y) => {
  ...
  if (
    ...
    //!= undefined
    if (newBoard[x][y].value !== SHIP_PART) {
      //making sure we aren't changing the value of a ship part to be an around ship bc of how loop works
...
const aroundShipBoardUpdate = (board, ship, newValue) => {
  let newBoard = [...board];
  const updater = (x, y) => {
    // used to update the relevant value in the same line, since both need to be around the entire ship
    if (newValue === AROUND_SINK) {
      //when ship is sunk
      newBoard = aroundSinkUpdate(board, x, y);
    } else if (newValue === AROUND_SHIP) {
      //when ship is placed so no ships will be placed 1 square around it
...
export const hitBoardUpdate = (x, y, shipIndex, board, ships) => {
  let newShips = [...ships];
  let newBoard = [...board];
  const partIndex = newShips[shipIndex].shipParts.findIndex(
  ...
  const isShipSunk = newShips[shipIndex].shipParts.every(
    (shipPart) => shipPart.isHit
  ); //checking if all parts of the ship are hit

  if (isShipSunk) {
    newShips[shipIndex].isSunk = isShipSunk; // updating the ship in the ships array as sunk
    newBoard = aroundShipBoardUpdate(
      newBoard,
      newShips[shipIndex],
      AROUND_SINK
    ); // updating the board around the ship as sunk
    console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
    console.table(newShips);
  }
  ...
  for (let loopX = x - 1; loopX <= x + 1; loopX++) {
    for (let loopY = y - 1; loopY <= y + 1; loopY++) {
      if ((loopX + loopY) % 2 === (x + y) % 2) {
        // this condition allows us to adress only squares diagonal to the square in [x][y]
...
export const placeShips = (board, ships) => {
  ...
  newShips.forEach((ship, index) => {
    // iterates over every ship until it is placed
    let needsPlacing = true;
    const directionalAdder = (limited, unlimited, num) => {
      // will be used to adress both vertical and horizontal facing ships in the same placement
    ...
    re_placeShips: while (needsPlacing) {
      // looks for a place for the ship until it finds it
      ...
      for (let i = 0; i < ship.length; i++) {
        // iterates over the generated place for the ship to check if its viable
...
export const initialShips = (gameType = RUSSIAN) => {
  const shipNames = [
    ...
    "S2",
    "S1",
  ]; //names of the ships in reverse order of giving them
  const ships = []; //the soon to be returned ships array
  const shipCreator = (length, name = shipNames.pop()) => {
    // default name is the last object of the mane array
    return {
      name,
      length,
      shipParts: [],
  ...
  if (gameType === RUSSIAN) {
    // the ships unique to russian var
    ships.push(shipCreator(2));
    ships.push(shipCreator(2));
    for (let i = 0; i < 4; i++) {
