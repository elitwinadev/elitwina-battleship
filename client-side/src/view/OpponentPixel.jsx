...
import { SEA, MISS, HIT, SINK, AROUND_SINK, SHIP_PART, BsContext } from "../stateManager/stateManager";
...
const [alreadyPlayed, setAlreadyPlayed] = useState(false);
  if (status === SEA) {
    return <OpponentSquare lock={true} onClick={() => clickhandler(x, y, lock)} ></OpponentSquare>
  }
  else if (status === MISS) {
    if (isAnimated) {
      if (!alreadyPlayed) {
    ...
    return <MissHit isAnimated={isAnimated} lock={lock} onClick={() => clickhandler(x, y, lock)} >•</MissHit>
  }
  else if (status === HIT) {
    if (isAnimated) {
      if (!alreadyPlayed) {
    ...
    return <ShipHit isAnimated={isAnimated} lock={lock} onClick={() => clickhandler(x, y, lock)} >X</ShipHit>
  }
  else if (status === SINK) {
    if (isAnimated) {
      if (!alreadyPlayed) {
    ...
    return <AroundSink isAnimated={isAnimated} lock={lock}>•</AroundSink>
  }
  else if (status === SHIP_PART) {
    //
    if (dev)
      return <ShipPart lock={true} onClick={() => clickhandler(x, y, lock)} ></ShipPart> // dev tool
    else
    //
      return <OpponentSquare lock={true} onClick={() => clickhandler(x, y, lock)} ></OpponentSquare>
  }
